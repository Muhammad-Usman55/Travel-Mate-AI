...
export function Weather({
  ...
  const chartTemps = displayTemperatures;
  const chartMin = Math.min(...chartTemps);
  const chartMax = Math.max(...chartTemps);
  const chartRange = Math.max(1, chartMax - chartMin);
  const chartW = 100;
  const chartH = 36;
  const chartPoints = chartTemps.map((temp, i) => {
    const x = (i / (chartTemps.length - 1)) * chartW;
    const y =
      chartH - ((temp - chartMin) / chartRange) * (chartH - 6) - 3;
    return [x, y] as const;
  });
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border bg-card text-card-foreground p-5 shadow-sm max-w-[480px]">
          ...
          <div
            className={cx(
              ...
              isDay
                ...
                : 'bg-indigo-100 text-indigo-500 dark:bg-indigo-950/50 dark:text-indigo-300',
            )}
          >
            {isDay ? (
      ...
      <svg
        ...
        aria-hidden
      >
        <path d={areaPath} fill="hsl(var(--brand) / 0.12)" />
        <path
          d={linePath}
          fill="none"
      ...
      <div className="flex flex-row justify-between">
        {displayTimes.map((time, index) => (
          <div key={time} className="flex flex-col items-center gap-1">
            ...
            <div className="size-1.5 rounded-full bg-[hsl(var(--brand))]" />
